...
namespace efibootmgrw {
    ...
    struct efi_load_option {
        u32 attributes;
        u16 file_path_length_list;
        lak::array<wchar_t, 4096 * 10> desc;
    ...
    auto res_main(int argc, const char **argv_ptr) -> lak::result<lak::monostate> {
        ...
        winapi::open_current_process_token(winapi::token_adjust_privileges | winapi::token_query)
                ...
                .and_then([&](auto) {
                    return winapi::get_firmware_env_var(L"Boot0000", winapi::efi_global_variable, lak::span<void> { reinterpret_cast<void*>(&e), sizeof(e) });
                })
                .if_ok([&](auto&& buf) {
                    // auto* e = reinterpret_cast<efi_load_option*>(buf.data());


                    fmt::print(L"Boot0000: {}\n", e.desc.data());
                })
